...
export default function BestellingDetails() {
  ...
  const [bestellingTTC, setBestellingTTC] =
    useState<BestellingByTrackAndTrace>();
  ...
  function handleNavigateWithParams(
    pathname: string,
    bTTC: BestellingByTrackAndTrace | undefined
  ) {
  ...
  useEffect(() => {
    async function fetchBestelling() {
      const bestellingData = await bestellingByBestellingId(
        Number(bestellingIdString)
      );
      if (bestellingData) {
  ...
  return (
    <Container maxW="70%" centerContent>
      <Text
        onClick={() => handleNavigate("/bestellingen")}
        _hover={{
          cursor: "pointer",
      ...
      <Flex direction={"column"} className={"bestellingOverzicht"}>
        ...
        <Flex className={"bestellingDetails"}>
          <Flex direction={"column"}>
            ...
            <Text fontWeight="bold">Besteld op</Text>
            <Text>{bestelling?.datumGeplaatst.toString()}</Text>
            <Text fontWeight="bold">Aankoopstatus</Text>
            <Text>{bestelling?.status}</Text>
          </Flex>
          <Spacer />
          <Flex direction={"column"}>
            <Text fontWeight="bold">Leveradres</Text>
            <Text>
            ...
            <Text>{bestelling?.leveradresLand}</Text>
            <Text fontWeight="bold">Track en trace code</Text>
            {bestelling?.trackAndTraceCode ? (
              <Link
                colorScheme="red"
                onClick={() =>
                  handleNavigateWithParams(`/track-and-trace`, bestellingTTC)
                }
                _hover={{
                  cursor: "pointer",
                  textDecoration: "underline",
              ...
              <Text>Track and Trace niet beschikbaar</Text>
            )}
          </Flex>
          <Spacer />
          <Flex direction={"column"}>
            <Text fontWeight="bold">Verpakking</Text>
            <Text>{bestelling?.doos.naam}</Text>
            <Text fontWeight="bold">Leverancier</Text>
            <Text>{bestelling?.leverancierBedrijf.naam}</Text>
            <Text fontWeight="bold">Kostenoverzicht</Text>
            <Text>{bestelling?.totalPrice.toFixed(2)} €</Text>
          </Flex>
        </Flex>
      </Flex>
    </Container>
